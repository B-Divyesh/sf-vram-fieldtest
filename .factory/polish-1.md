...
|---|---|---|
| F-1-1 | The 390 px layout now keeps the copy, sample action, and facts before the bench image. The primary action links directly to `/?demo=1`. | `@claim:mobile-first-action`; `artifacts/polish-1-mobile-390x844.png`; live `/` check after deployment. |
| F-1-2 | Added `.factory/claims.json` entries and isolated checks for account-free use, output directory, non-invasive operation, unsigned packages, report price/free core, demo equality, demo storage, mobile first action, and route metadata. Narrowed removed promises where no useful observable proof exists. | Every exact claims command; clean-clone log in handoff. |
| F-1-3 | The static builder now emits distinct title, description, canonical, Open Graph, and Twitter values for each physical route and 404. The dev verifier serves that built output. | `@claim:route-metadata`; live `/demo`, `/report-kit`, `/privacy`, `/terms`, and missing-route header checks. |
