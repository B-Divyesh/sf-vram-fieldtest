...
const hero = await readFile(resolve(root, 'site/public/hero-vram-small.webp'));
const heroName = `hero-vram.${hash(hero)}.webp`;
const appSource = await readFile(resolve(root, 'site/src/app.js'), 'utf8');
const app = Buffer.from(appSource.replaceAll('/hero-vram-small.webp', `/assets/${heroName}`));
...
await writeFile(resolve(assetDir, heroName), hero);
...
worker = worker.replaceAll('/app.js', `/assets/${appName}`).replaceAll('/styles.css', `/assets/${cssName}`).replaceAll('/hero-vram-small.webp', `/assets/${heroName}`);
await writeFile(resolve(out, 'sw.js'), worker);
await unlink(resolve(out, 'app.js'));
await unlink(resolve(out, 'styles.css'));
await unlink(resolve(out, 'hero-vram-small.webp'));
