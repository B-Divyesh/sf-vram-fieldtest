...
try {
  for (const route of ['/', '/demo', '/report-kit', '/privacy', '/terms', '/missing-page-live-smoke']) {
    ...
    page.on('console', message => { if (message.type() === 'error') summary.consoleErrors.push(`${route}: ${message.text()}`); });
    page.on('pageerror', error => summary.consoleErrors.push(`${route}: ${error.message}`));
    const response = await page.goto(new URL(route, base), { waitUntil: 'networkidle' });
  ...
  await mobile.addStyleTag({ content: 'html{font-size:200%}' });
  ...
  await demoPage.goto(new URL('/demo', base), { waitUntil: 'networkidle' });
