...
struct RunArgs {
    ...
    #[arg(long, default_value_t=DEFAULT_MIB)]
    mib: u64,
    /// Do not start another pattern after this many seconds.
    ...
    match cli.command.unwrap_or(Cmd::Inspect { json: false }) {
        Cmd::Inspect { json } => {
            ...
            if json {
                ...
                if let Some(m) = a.detected_vram_mib {
                    println!("Detected VRAM: {m} MiB ({})", a.source);
                } else {
                    println!("VRAM: unavailable ({})", a.source);
                }
            }
        }
        Cmd::Demo { json } => {
            let dir =
                std::env::temp_dir().join(format!("vram-fieldtest-demo-{}", std::process::id()));
...
fn run(args: RunArgs) -> Result<()> {
    ...
    if args.mib == 0 || args.mib > MAX_MIB {
        bail!("Not started: --mib must be between 1 and {MAX_MIB} MiB.");
    }
    let started = Utc::now();
    let clock = Instant::now();
    let (gpu, adapter) = open_gpu()?;
    let safe_mib = (gpu.max_mib).min(args.mib);
    if safe_mib < args.mib {
        eprintln!("Requested {} MiB; this adapter exposes {} MiB for one safe storage buffer. Coverage is reported from the actual pass.",args.mib,safe_mib);
    }
    ...
    for (name, word) in [
        ...
        if clock.elapsed().as_secs() >= args.seconds {
            bail!("Stopped at the {} second time limit before {name}. Lower --mib or allow more time.", args.seconds);
        }
        if let Some(temp) = gpu_temperature() {
            if temp >= thermal_limit {
                bail!("Stopped before {name}: GPU temperature is {temp}°C, at or above the {thermal_limit}°C limit.");
            }
        }
        let result = exercise_gpu_buffer(&gpu, bytes, word, name)?;
        if let Some(temp) = gpu_temperature() {
            if temp >= thermal_limit {
                bail!("Stopped after {name}: GPU temperature is {temp}°C, at or above the {thermal_limit}°C limit.");
            }
        }
    ...
    let coverage = adapter
        .detected_vram_mib
        .map(|v| (safe_mib as f64 / v as f64 * 100.0).min(100.0));
    let status = if bad == 0 { "pass" } else { "fail" };
    let summary = if bad == 0 {
        format!(
            "No mismatches in {} MiB of GPU buffer space across {} patterns.",
            safe_mib,
            patterns.len()
        )
    } else {
        format!("{bad} mismatches found. Do not rely on this GPU until it is retested.")
    };
    ...
    if args.json {
        println!(
            "{}",
            serde_json::json!({"output":args.output,"status":status,"coverage_percent":coverage,"mismatches":bad})
        );
    } else {
        println!(
            "{}\nJSON: {}\nPrint report: {}",
...
fn open_gpu() -> Result<(Gpu, Adapter)> {
    ...
    let max_mib = (limits.max_storage_buffer_binding_size as u64 / 1024 / 1024)
        .min(MAX_MIB)
        .max(1);
...
fn exercise_gpu_buffer(gpu: &Gpu, bytes: u64, word: u32, name: &str) -> Result<Pattern> {
    ...
    let output = gpu.device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("field test memory"),
        size: bytes,
        usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC,
        mapped_at_creation: false,
    });
    let readback = gpu.device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("field test readback"),
        size: bytes,
        ...
        .create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
    {
        ...
        pass.dispatch_workgroups(((words + 63) / 64) as u32, 1, 1);
    }
    ...
    rx.recv()
        .context("GPU read-back did not finish")?
        .context("GPU could not map test data")?;
...
fn gpu_name() -> Option<String> {
    #[cfg(target_os = "linux")]
    {
        if let Ok(o) = Command::new("sh")
            .args(["-c", "lspci 2>/dev/null | grep -Ei 'vga|3d' | head -1"])
            .output()
        {
...
fn render_html(r: &Report) -> String {
    let rows = r
        ...
        .map(|p| {
            format!(
                "<tr><td>{}</td><td>{} MiB</td><td>{}</td><td>{}</td></tr>",
                esc(&p.name),
                p.bytes / 1024 / 1024,
                p.mismatches,
                esc(&p.status)
...
mod tests {
    use super::*;
    #[test]
    fn local_pattern_rule_is_sound() {
        assert_eq!((33u32) ^ 0xA5A5_5A5A, (33u32) ^ 0xA5A5_5A5A);
    }
    #[test]
    fn html_includes_coverage() {
        let r: Report =
            serde_json::from_str(include_str!("../examples/sample-report.json")).unwrap();
        assert!(render_html(&r).contains("Coverage"));
    }
}
